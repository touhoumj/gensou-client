...
local poll = function(self)
    ...
    if decoded then
        ...
        elseif not self.frames then
            local return_message = decoded
            local return_opcode = self.first_opcode
            receive_init(self)
            return return_message, return_opcode
        else
            tinsert(self.frames, decoded)
            local return_message = tconcat(self.frames)
            local return_opcode = self.first_opcode
            receive_init(self)
            return return_message, return_opcode
        end
...
local connect = function(self, ws_url, ws_protocol, ssl_params)
    ...
    if protocol == "wss" then
        self.sock, msg = ssl.wrap(self.sock, ssl_params)
        print(self.sock:dohandshake())
